...
/*  Create CSS */
    var styles = '\
        ...
        .ggs-hidden .ggs-guide, .ggs-hidden #ggs-baseline-container, .ggs-hidden #ggs-container {display: none;}\
        .ggs-0{left:0;}\
...
/* 	Create guides */
	ender('body').append(ender('<div id="ggs-container"></div>'));
	for (i=0; i<=16; i++) {
		ender('#ggs-container').append(ender('<div class="ggs-guide ggs-'+i+'"><div></div></div>'));
	};
...
/* 	Append CSS */
	...
	ender(window).resize(function() {
		ender('#ggs-container').width(ender('body').width() - 80);
	});
	ender('#ggs-container').width(ender('#ggs-container').width() - 80);
